...
function EventModal(props) {
  ...
  const getAppointments = (day, month) => {
    const dayAppointments = appointmentsList
      .filter(
        (task) =>
          new Date(task.time).getDate() === day &&
          new Date(task.time).getMonth() === month
      )
      .sort((a, b) => {
    ...
    if (addApBol === true) {
      if (selectedTime) {
        ...
        localStorage.setItem(
          "appointmentsList",
          JSON.stringify(updatedAppointmentsList)
        );
      } else {
        ...
        <Modal.Body>
          ...
          {addApBol === true ? (
            <>
              ...
              {notTimeSelected === true ? (
                <p className="alert alert-danger text-sm my-3 p-2">
                  Select a time.
                </p>
              ) : (
                <></>
