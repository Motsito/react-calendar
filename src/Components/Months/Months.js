...
export default function Months() {
  ...
  const boxClick = (index, currentBox, currentDays) => {
    //if that allow us to open the modal only when second click gets recieved
    let isItTrue = selectedDay === index;

    if (!!isItTrue) {
      setShowModal(true);
    }

    //if that allow us to select only days with white background
  ...
  const getClassNames = (
    ...
    const classNames = {
      ...
      today:
        date.getDate() === currentBox - firstDay &&
  ...
  const getDayNumber = (index, currentBox, currentDays, pastMonthDays) => {
  ...
  const getAppointments = (index, day, month) => {
    return appointmentsList.map((task) => {
      ...
      if (taskMonth === month && taskDay === day) {
        return (
          <span className="dayText" key={Math.floor(Math.random() * index)}>
            {task.name}
          </span>
        );
      }
  ...
  const makeDays = () => {
    return monthboxes.map((_, index) => {
      ...
      const notAvailableDaysHelper =
        currentBox <= firstDay ||
        currentBox < firstDay ||
        currentBox > currentDays + firstDay;
      let pastMonthDays = () => {
        if (currentMonth > 0 && currentMonth <= 11) {
      ...
      return (
        <div
          ...
          key={index}
          onClick={() => {
            boxClick(index, currentBox - firstDay, currentDays);
          }}
        >
          <p>{getDayNumber(index, currentBox, currentDays, pastMonthDays())}</p>
          <div>
